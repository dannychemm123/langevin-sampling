...
def imshow_density(density: Density, bins:int, scale:float=4.0, ax: Optional[Axes] = None, **kwargs):
    if ax is None:
        ax = plt.gca()
    # Extract x_bounds and y_bounds from kwargs if present, otherwise use scale
    x_bounds = kwargs.pop('x_bounds', [-scale, scale])
    y_bounds = kwargs.pop('y_bounds', [-scale, scale])
    x = torch.linspace(x_bounds[0], x_bounds[1], bins).to(device)
    y = torch.linspace(y_bounds[0], y_bounds[1], bins).to(device)
    X, Y = torch.meshgrid(x, y)
    xy = torch.stack([X.reshape(-1), Y.reshape(-1)], dim=1)
    density_vals = density.log_density(xy).reshape(bins, bins).T
    im = ax.imshow(density_vals.cpu(), extent=[x_bounds[0], x_bounds[1], y_bounds[0], y_bounds[1]], origin='lower', **kwargs)
